...
SELECT l.name, creation_time, starting_price, image_link, c.name FROM lots l JOIN categories c ON l.category_id = c.id
WHERE end_time > now() ORDER BY creation_time DESC;
